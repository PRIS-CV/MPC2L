...
class Market1501(object):
    ...
    """
    dataset_dir = 'market1501/raw/Market-1501-v15.09.15'

    def __init__(self, root='data', aug_dir='train_augment', **kwargs):
        self.dataset_dir = osp.join(root, self.dataset_dir)
        self.train_dir = osp.join(self.dataset_dir, 'bounding_box_train')
        self.query_dir = osp.join(self.dataset_dir, 'query')
        self.gallery_dir = osp.join(self.dataset_dir, 'bounding_box_test')
        self.augment_dir = osp.join(self.dataset_dir, aug_dir)

        self._check_before_run()
        self.with_augment = osp.exists(self.augment_dir)
        ...
        if self.with_augment:
            print("augment data dir: " + aug_dir)
            augment, num_augment_pids, num_augment_imgs = self._process_dir(self.augment_dir, relabel=True, label_start=num_train_pids)
            self.augment = augment
            self.num_augment_pids = num_augment_pids
        ...
        self.num_cameras = np.max([x[2] for x in train]) + 1
